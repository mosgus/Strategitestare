...
def run_assignment(
    ...
    results_m = []
    for m_profit in range(m_min, m_max + 1, m_step):
        wins, prob, exp = simulate_point(
            fixed_n,
            m_profit,
            iterations,
    ...
    with open(f'assignment_data/fixed_M_{fixed_m}.csv', 'w', newline='') as f:
    ...
    with open(f'assignment_data/fixed_N_{fixed_n}.csv', 'w', newline='') as f:
        writer = csv.DictWriter(
            f,
            fieldnames=['N', 'M_profit', 'Wins', 'Iterations', 'Prob_Win', 'Expected_Return'],
        )
...
def parse_args():
    ...
    parser.add_argument('--n-step', type=int, default=1)  # N sweep increment
    ...
    parser.add_argument('--m-step', type=int, default=1)  # M sweep increment
    parser.add_argument('--iterations', type=int, default=1)  # Runs per (N, M); increase for smoother probabilities.
